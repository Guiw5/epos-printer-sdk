...
export function getBoolAttr(name: string, value: boolean): string {
  return ` ${name}="${!!value}"`;
}
...
export function getEnumIntAttr(name: string, value: any, regex: RegExp, min: number, max: number): string {
  if (!regex.test(value)) {
    if (isNaN(value) || value < min || value > max) {
      throw new Error(`Parameter "${name}" is invalid`);
    }
